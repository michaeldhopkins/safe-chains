...
pub fn classify_locus(path: &str) -> LocalLocus {
    ...
    // Unresolvable — could name anything, so worst-case to `machine` (the top fs rung;
    // raw devices need an explicit /dev/ match, so they are not assumed here).
...
mod tests {
    ...
    #[test]
    fn a_dollar_or_escape_dominates_an_otherwise_worktree_shape() {
        // conservative: an unpinnable segment can't be trusted as worktree-local
